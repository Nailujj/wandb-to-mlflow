...
| logged artifacts (`run.logged_artifacts()`) | artifacts under `artifacts/<name>/` | **lossy** | Opt-in via `--artifacts`. Bytes are copied; **versions, aliases, lineage, types and metadata are not**. A JSON sidecar `artifacts/<name>/_wandb_artifact.json` records name, version, aliases and digest as inert text. |
