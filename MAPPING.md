...
| system metrics (`run.history(stream="system")`) | metrics prefixed `system.` | **lossy** | Opt-in via `--system-metrics`. W&B samples these server-side and offers no exhaustive reader for the stream — `scan_history` takes no `stream` argument at all — so the migrated series is the sampled one, not the raw one. We ask for far more samples than any real run has. W&B already names these keys `system.*`; the prefix is applied idempotently, so a key it did not prefix still gets one and one it did is not doubled. |
| Reports | — | **no** | |
